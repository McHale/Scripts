...
local trashex=false
local smith=false
local loottp={9914,3939,5046,5127,5177,10159,9916,5040,10148,5049,10155,5123,
            9918,10146,5179,3932,8820,10151,5181,9919,10153,3922,5185,9921,
            3934,9915,9917,10152,5112,3721,3713,10150,5042,9922,3920,5117,
            9923,10149,3780,3719,5044,3779,5110,3568}
local lootar={4230,4231,4234,5051,5054,5055,5100,5099,5105,5104,5128,
             5132,5134,5136,5137,5138,5139,5140,5141,5147,5198,5199,
             5200,5201,5202,10112,10113,10157,11119}
local loota={{str="Mana Regen",int=2},
             {str="Damage Increase",int=20},
             {str="Hit Chance",int=15},
             {str="Reagent Cost",int=15},
             {str="Mana Cost",int=15},
             {str="Dexterity Bonus",int=7},
             {str="Strength Bonus",int=7},
             {str="Defence Chance",int=15},
             {str="Swing Speed",int=15}}
local loot={{str="Mana Regen",int=2},
						{str="Damage Increase",int=35},
						{str="Strength Bonus",int=7},
						{str="Dexterity Bonus",int=7},
						{str="Resist",int=8},
						{str="Area",int=35},
						...
						{str="Harm",int=35},
						{str="Fireball",int=35},
						{str="Lightning",int=35},
						{str="Lower Attack",int=35},
						{str="Lower Defense",int=35}}
...
local tool={2431,3718,3722,3739,3897,3907,3909,3911,3913,3915,3917,3997, 
                  ...
                  4979,5040,5091,5092,5115,5182,5187,7866,7864}
local wood={7121,7124,7127,7133,7163,3903}
local tailor={3613,3576,4225,5990,5991,6112,9908}
local metal={7154}
local rock={6009}
local magic={2426,3827,3960,3962,3963,3965,3966,3968,3969,3972,
                     3973,3974,3976,3978,3980,3981,3982,3983,4586,5154,7956}
...
local trash={2504,2600,2860,2861,3307,3309,3310,3311,3312,3313,3314,3568,3570,3571,3572,3573,
             3719,3779,3780,3847,3848,3849,3850,3851,3852,3853,3971,4039,4110,4179,
             5044,5110,5147,5198,5199,5200,5201,5202,5365,5451,6217,6218,
...
local cut={3989,4217,5062,5063,5067,5068,5069,5077,5078,5082,5083,5084,
           ...
           8059,8095,8097,8189,8970,8972,8976,
           10102,10110,10114,10122,10126,10127,10129,
           10130,10131,10132,10134,10135,10136,10145,10182,10183,
           11124,11125,11127,11128}                
local boxchop={3709,3647,3713,3721,5112,10150,10152}
local chop={5042,5117,9922,9923}
local toolc,woodc,magicc,tailorc,metalc,rockc,gemc=0,0,0,0,0,0,0
local trashc,bypassc,maglistc,tralistc,cutc,smeltc,chopc,boxchopc=0,0,0,0,0,0,0,0
local tbag,key,ch,bch,saved={},{},{},{},{}
...
for i,ii in pairs(tbl) do
	local bol=true
      if next(saved) then
         for t,tt in pairs(saved) do
             if tt==ii.id then bol=false break end
         end
      end
	if bol then
...
if next(ch) or next(bch) and key[7]~=nil then storit(7)  end
...
local c,tr,sm={},{},{}
local result,bol=false,true
for i,ii in pairs(tbl) do
    for t,tt in pairs(loottp) do
        if tt==ii.tp then
        
           local _,inf=UO.Property(ii.id)
           print(inf)
           for c,cc in pairs(loot) do
               local num=string.find(string.lower(inf),string.lower(tostring(cc.str)))
               if num then 
                  local int=tonumber(string.match(inf,"%d+",num))
                  UO.SysMessage(tostring(int))
                  if int>=cc.int then UO.SysMessage(tostring(cc.str).." "..string.match(inf,"%d+",num)) 
                  table.insert(saved,ii.id) break end
               end
           end
           break
        end
	...
	for t,tt in pairs(trash) do
      if ii.tp==tt then 
			...
			elseif t>cutc and t<=chopc then print("CH "..tt)
				table.insert(ch, ii.id)
				print(UO.Property(ii.id))
			elseif t>chopc and t<=boxchopc then print("BCH "..tt)
			print(UO.Property(ii.id))
				table.insert(bch, ii.id)
			end
			bol=false
			break
		end
...
UO.LObjectID=key[int]
 if int==7 then
   local tmp=0
   if next(bch) then tmp=1 end
   if next(ch) then tmp=tmp+2 end
   if tmp>0 then chopit(tmp) end
...
while true do
   ...
   timer=getticks()+500
   while timer>getticks() do
      if UO.ContSizeX==gump[7].sx and UO.ContSizeY==gump[7].sy then
      if int==1 then UO.Click(UO.ContPosX+85, UO.ContPosY+160, true, true, true, false)
         table.insert(tmp,4148)
      elseif int==2 then UO.Click(UO.ContPosX+85, UO.ContPosY+195, true, true, true, false)
             table.insert(tmp,4130)
      else UO.Click(UO.ContPosX+85, UO.ContPosY+160, true, true, true, false)
      table.insert(tmp,4148)
         timer=getticks()+500         
         while timer>getticks() do end
         UO.Click(UO.ContPosX+85, UO.ContPosY+195, true, true, true, false)
         table.insert(tmp,4130)
      end
      bol=true break 
      end
...
local result={}
 for i=0, UO.ScanItems(false)-1,1 do
      local nid,typ,_,contid,_,_,_,stck=UO.GetItem(i)
        for t,tt in pairs(tmp) do
        if typ==tt and contid==UO.BackpackID then table.insert(result,nid) print(nid)
           table.insert(tl,nid) break end
        end
        if next(result) then
           if #result==#tmp then break end
        end
end
for i=1,#tmp do  
    local bol=false
    while true do
    UO.LObjectID=result[i]
    UO.Macro(17,0)
    local timer=getticks()+500
    while timer>getticks() do
          if UO.ContSizeX==530 and UO.ContSizeY==437 then bol=true break end
    end
    if bol then break end
    end
    if tmp[i]==4130 then
       for t,tt in pairs(ch) do
       UO.LTargetKind=1
       UO.LTargetID=tt
       while not UO.TargCurs do
             UO.Click(UO.ContPosX+30,UO.ContPosY+350,true,true,true,false)
             wait(350)
       end
      UO.Macro(22,0)
      local timer=getticks()+500
...
while UO.ContSizeX==530 do UO.Click(UO.ContPosX+UO.ContSizeX/2,UO.ContPosY+UO.ContSizeY/2, false, true, true, false) end      
wait(500)
while UO.ContSizeX==530 do UO.Click(UO.ContPosX+UO.ContSizeX/2,UO.ContPosY+UO.ContSizeY/2, false, true, true, false) end
end
...
for i,ii in pairs(chop) do table.insert(trash,ii) end
chopc=cutc+#chop
for i,ii in pairs(boxchop) do table.insert(trash,ii) end
boxchopc=chopc+#boxchop
...
wand 3            3572
wand 4            3573
wooden bench      2860,2861
wooden shield     7034
local trash={2504,2600,2860,2861,3307,3309,3310,3311,3312,3313,3314,3535,3568,3570,3571,3572,3573,
             3718,3719,3779,3780,3847,3848,3849,3850,3851,3852,3853,3971,4230,4231,42344039,4179,
             5044,5110,5117,5147,5198,5199,5200,5201,5202,5365,5451,5981,6217,6218,
             7033,7034,7583,7584,7585,7587,7588,7848,7947,7964,7981,7982,7983,7984,7985,7986,7987,7988,7989,
             7990,7991,7992,7993,7994,7995,7996,7997,7998,7999,
             8000,8001,8002,8003,8004,8005,8006,8007,8008,8009,
             8010,8011,8013,8014,8015,8016,8017,8018,8019,
...
local equip={4230,4231,4234,5051,5054,5055,5100,5099,5105,5104,5128,
             5132,5134,5136,5137,5138,5139,5140,5141,5147,5198,5199,
             5200,5201,5202,10112,10113,10157,11119}
local weps={9914,3939,5046,5127,5177,10159,9916,5040,10148,5049,10155,5123,
            9918,10146,5179,3932,8820,10151,5181,9919,10153,3922,5185,9921,
            3934,9915,9917,10152,5112,3721,3713,10150,5042,9922,3920,5117,
            9923,10149,3780,3719,5044,3779,5110,3568}
-------CUT--------
local cut={3989,4217,5062,5063,5067,5068,5069,5077,5078,5082,5083,5084,
           5134,5397,5398,5399,5422,5431,5433,5435,5437,5440,5441,5444,5899,           
           5901,5903,5905,5907,5908,5909,5910,5912,5913,5914,5915,5916,
           7168,7170,7174,7176,7178,7180,7609,7933,7937,7939,
           8059,8095,8097,8189,8970,8972,8976,
           10102,10110,10114,10122,10126,10127,10129,
           10130,10131,10132,10134,10135,10136,10145,10182,10183,
